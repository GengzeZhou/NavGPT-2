...
class Blip2T5InstructNav(Blip2Base):
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            # Get image tokens
            self.image_token_id = self.t5_tokenizer.convert_tokens_to_ids(['<image>'])[0]
            all_image_indices = (input_tokens.input_ids == self.image_token_id).to(inputs_t5.device)

            assert (input_tokens.input_ids[all_image_indices].shape[0] == inputs_t5.shape[0] * inputs_t5.shape[1]), \
            ...
            assert (inputs_t5.shape[-1] == inputs_embeds.shape[-1]), f"{inputs_t5.shape[-1]} != {inputs_embeds.shape[-1]}"

            inputs_t5 = inputs_t5.reshape(-1, inputs_t5.shape[-1]).to(inputs_embeds.dtype)
