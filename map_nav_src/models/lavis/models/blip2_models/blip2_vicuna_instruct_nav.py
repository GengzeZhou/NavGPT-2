...
@registry.register_model("blip2_vicuna_instruct_nav")
class Blip2VicunaInstructNav(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    Usage:
        ...
        >>> model = load_model("blip2_vicuna_instruct_nav", "vicuna7b")
    """

    PRETRAINED_MODEL_CONFIG_DICT = {
        "vicuna7b": "configs/models/blip2/blip2_instruct_nav_vicuna7b.yaml",
        "vicuna13b": "configs/models/blip2/blip2_instruct_nav_vicuna13b.yaml",
    }
    ...
    def __init__(
        ...
        # add special tokens
        num_new_tokens = self.llm_tokenizer.add_special_tokens(
            {   
                "additional_special_tokens": [DEFAULT_IMG_TOKEN, DEFAULT_IMG_END_TOKEN, DEFAULT_IMAGE_TOKEN, DEFAULT_INSTRUCTION_TOKEN, DEFAULT_INSTRUCTION_END_TOKEN],
                'bos_token': '</s>',
                'eos_token': '</s>',
                'unk_token': '</s>',
                'pad_token': '[PAD]', 
    ...
    def forward(self, samples):
        ...
        # Get image tokens
        self.image_token_id = self.llm_tokenizer.convert_tokens_to_ids([DEFAULT_IMAGE_TOKEN])[0]
        all_image_indices = (llm_tokens['input_ids'] == self.image_token_id).to(image.device)

        assert (llm_tokens['input_ids'][all_image_indices].shape[0] == inputs_llm.shape[0] * inputs_llm.shape[1]), \
        f"image tokens in input ids {llm_tokens['input_ids'][llm_tokens['input_ids'] == self.image_token_id].shape[0]} != the number of image tokens {inputs_llm.shape[0]}"
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            all_image_indices = (llm_tokens.input_ids == self.image_token_id).to(image.device)
            assert (llm_tokens.input_ids[all_image_indices].shape[0] == inputs_llm.shape[0] * inputs_llm.shape[1]), \
            f"image tokens in input ids {llm_tokens.input_ids[all_image_indices].shape[0]} != the number of image tokens {inputs_llm.shape[0]}"
            assert (inputs_llm.shape[-1] == inputs_embeds.shape[-1]), f"{inputs_llm.shape[-1]} != {inputs_embeds.shape[-1]}"
            inputs_llm = inputs_llm.reshape(-1, inputs_llm.shape[-1]).to(inputs_embeds.dtype)
            inputs_embeds[all_image_indices] = inputs_llm
