...
@registry.register_processor("blip_navgpt_prompt")
class BlipPromptProcessor(BaseProcessor):
    def __init__(self, prompt=""):
        self.prompt = prompt

    def __call__(self, ann):
        prompt = self.pre_prompt(ann)
    ...
    def pre_prompt(self, ann):
        ...
        prompt = self.prompt
        prompt = prompt.replace("{instruction}", ann['instruction'])
        candidate_dict = {}
        for i, can in enumerate(ann['candidate'].values()):
            heading = can['heading']
            if -45 <= heading <= 45:
                direction = "front"
            elif 45 < heading <= 135:
            ...
            # key = f"Candidate {i}, facing {can['heading']:.2f} degrees, {can['distance']:.2f} meters"
            candidate_dict[key] = image_p
        prompt = prompt.replace("{candidate}", str(candidate_dict))
        # prompt = prompt.replace("{history}", str(ann['history']))
        return prompt
